...
> **Known gap:** four of the components this route depends on
> (`AuthorSection`, `FaqSection`, `NewsletterForm`, `TableOfContents`) don't
> yet have a `guides/blogs/*.md` of their own. Everything about them below is
> reconstructed from their `.tsx` source directly. Worth writing those four
> guides to match the other nine — ask if you want them drafted.
...
|---|---|---|---|
| 1 | `AuthorSection.tsx` | C — direct render | *(no guide yet)* |
| 2 | `Button.tsx` (`BlogButton` / `injectBlogButtons`) | A (marker) + direct JSX use | `guides/blogs/button.md` |
| 3 | `FaqSection.tsx` | A — static | *(no guide yet)* |
| 4 | `ImagesGallery.tsx` | B — portal | `guides/blogs/image-gallery.md` |
| 5 | `NewsletterForm.tsx` | B — portal | *(no guide yet)* |
| 6 | `ProductGallery.tsx` (shoppable embeds: single/solo/duo/trio) | B — portal | *(referenced as `docs/shoppable-blog-articles.md`, outside `guides/`)* |
...
| 12 | `TableOfContents.tsx` | C — direct render | *(no guide yet)* |
...
// 11. Heading IDs / TOC — must run after all the above, since new
//     headings could theoretically be introduced by upstream transforms,
//     and it needs to scan the FINAL html for h2/h3 tags.
...
- 4 components have no dedicated guide: `AuthorSection`, `FaqSection`,
  `NewsletterForm`, `TableOfContents`. These are arguably the most
  wiring-heavy of the 14 (metafield gating chains, a `<script>`-tag marker
  syntax unlike anything else in the system, real backend POST handling,
  and heading-tree/scroll-spy logic respectively) — worth writing next.
- `ProductGallery.tsx`'s reference doc (`docs/shoppable-blog-articles.md`)
  lives outside `guides/blogs/` entirely and hasn't been reviewed in this
  thread.
